...
export class DiscordHandler implements Bootable {
  ...
  constructor(
    ...
    container.register<Discord.Client>(Discord.Client, {
  ...
  onReady() {
    ...
    this.log.info(
      `To allow this bot onto your server, go to the following url:`
    );
    this.log.info(
      `https://discordapp.com/api/oauth2/authorize?client_id=${this.client.user?.id}&scope=bot&permissions=103890496`
    );
  ...
  onMessage(msg: Discord.Message) {
    ...
    const parser = new Parser(
      this.env.get("DISCORD_COMMAND_PREFIX"),
      msg.content
    );
    ...
    if (
      ...
      msg.channel.name.indexOf("bumblebee") >= 0 &&
      !command
    ) {
